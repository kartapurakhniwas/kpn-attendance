...
@Component({
  ...
  template: ` 
   <i title="Copy Container" (click)="CopyData(orderId)" class="ki-duotone ki-copy"></i>  <a href="javascript:void(0)" (click)="redirect()" style="color: blue;">{{cellValue}}</a>
  `,
})


export class ContainerRedirect implements ICellRendererAngularComp {
...
CopyData(orderId:any)
{
  this._srvContainerTrack.GetContainernos(orderId).subscribe((m: any) => {
    if (m.success) {
      if(Array.isArray(m.lstModel))
        {
          let data=  m.lstModel.join();
          this.clipboard.copy(data);
          this._snackBar.open("Copied", "Okay", { 'duration': 3000 });
        }
     
     console.log(m,'GetContainernos')
    }
  });
  ...
  redirect()
  {
    if(this.orderType==1)
      {
        this.nav.navigateByUrl('/import-orders/details/'+this.orderGuid);
      }
   else
   {
    this.nav.navigateByUrl('/export-orders/details/'+this.orderGuid);
   }
  }
